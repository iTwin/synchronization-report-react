...
export const ReportHeader = (props: ReportHeaderProps) => {
  const { data, serviceReportUrl, onCreateServiceRequestClick } = props;
  ...
  useEffect(() => {
    ...
    setDate(data?.context?.timestamp ? new Date(data.context.timestamp).toLocaleDateString(undefined, options) : '');
  }, [data]);
    ...
    <>
      <div className='isr-report-header'>
        <div className='isr-header-title-wrapper'>
          <div className='isr-header-title'>
            {'Synchronization Report | ' + data?.sourceFilesInfo?.fileName ?? 'unknown'}
          </div>
